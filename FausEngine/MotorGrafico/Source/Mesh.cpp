...
Mesh::Mesh()
{
	mVBO = 0; mVAO = 0; meshLoaded = false;
...
void Mesh::Render()
{
	//shaderID = FausEngine::FsGame::GetSelfReference()->GetShader()->getProgram();
	shaderID = material.GetShader()->GetShaderId();
	...
	glUniformMatrix4fv( glGetUniformLocation(shaderID,"model"),	1, GL_FALSE, glm::value_ptr(mModel));

	//Material setting
	glUniform3f(glGetUniformLocation(shaderID,"material.ambient"),
		material.ambient.x, material.ambient.y, material.ambient.z);
	glUniform3f(glGetUniformLocation(shaderID,"material.specular"),
		material.specular.x, material.specular.y, material.specular.z);
	glUniform3f(glGetUniformLocation(shaderID, "color"),
		material.color.x, material.color.y, material.color.z);
	glUniform1f(glGetUniformLocation(shaderID,"material.shininess"), material.shineness);
	glUniform1i(glGetUniformLocation(shaderID,"material.texture"), 0);
	glUniform1i(glGetUniformLocation(shaderID,"lit"), true ? material.type == TypeMaterial::Lit : false);
