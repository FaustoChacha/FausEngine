...
void MouseControl(float xchange, float yChange, FsCamera* cam) {
...
void Escenario1::ControlesJugador(float dt, float time) {
	...
	if (gamereference->GetKeys()[87]) { // tecla W
		FsVector3 posicion =
			...
			gamereference->GetCamera()->Forward() * dt * velocidad;
		gamereference->GetCamera()->SetPosition(posicion);
	}
	if (gamereference->GetKeys()[83]) { // tecla S
		FsVector3 posicion = gamereference->GetCamera()->GetCameraPosition() -= gamereference->GetCamera()->Forward() * dt * velocidad;
		gamereference->GetCamera()->SetPosition(posicion);
	}
	if (gamereference->GetKeys()[65]) { // tecla A
		FsVector3 posicion = gamereference->GetCamera()->GetCameraPosition() -= gamereference->GetCamera()->Right() * dt * velocidad;
		gamereference->GetCamera()->SetPosition(posicion);
	}
	...
	if (gamereference->GetKeys()[69]) { // tecla E
		FsVector3 posicion = gamereference->GetCamera()->GetCameraPosition() += gamereference->GetCamera()->Up() * dt * velocidad;
		gamereference->GetCamera()->SetPosition(posicion);
	}
